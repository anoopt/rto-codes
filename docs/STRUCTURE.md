...
flowchart TB
    ...
    subgraph Scripts ["⚙️ Build Scripts"]
...
Static data for OpenStreetMap-based maps. Generated by OSM scripts.

| File                       | Description                                                                    |
| -------------------------- | ------------------------------------------------------------------------------ |
| `{state}/boundaries.json`  | **AUTO-GENERATED.** GeoJSON district boundary polygons for map rendering.      |
| `{state}/coordinates.json` | **AUTO-GENERATED.** RTO coordinates for map markers. Generated by OSM scripts. |
...
- `components/OSMStateMap.tsx` — Uses `districtMapping` for district boundary lookups
...
| `districtMapping` | Maps district names → OSM boundary IDs for interactive map highlighting |
| `osmEnabled` | Enable OpenStreetMap-based maps for this state |
...
```mermaid
flowchart LR
    subgraph Without ["❌ WITHOUT index.json"]
        direction TB
...
### 🗺️ OSM Map Data Scripts

These generate static data for OpenStreetMap-based district maps. Run manually for each state.

| Script                    | Command                                              | Purpose                                                     |
| ------------------------- | ---------------------------------------------------- | ----------------------------------------------------------- |
| `generate-boundaries.ts`  | `bun scripts/generate-boundaries.ts --state=<name>`  | Fetches district boundary polygons from Nominatim API.      |
| `generate-coordinates.ts` | `bun scripts/generate-coordinates.ts --state=<name>` | Fetches RTO coordinates for map markers from Nominatim API. |

**Output:** `public/data/{state}/boundaries.json` and `public/data/{state}/coordinates.json`

> **Note:** These scripts respect Nominatim's 1 request/second rate limit. Use `--force` to regenerate existing files.
...
| Component            | Purpose                                                                   |
| -------------------- | ------------------------------------------------------------------------- |
| `OSMStateMap.tsx`    | OpenStreetMap-based interactive map with district boundaries and markers. |
| `OSMDistrictMap.tsx` | OSM map for single district view with RTO markers.                        |
...
| `state-config.ts`   | State configuration helpers: `getStateConfig()`, `getDistrictMapping()`.          |
| `cloudinary.ts`     | Cloudinary URL generation for RTO images.                                         |
| `osm-boundaries.ts` | Loads district boundary polygons from static JSON for OSM maps.                   |
| `osm-geocoding.ts`  | Loads RTO coordinates from static JSON for map markers.                           |
| `feature-flags.ts`  | Feature flag helpers for OSM maps: `isOSMEnabled()`, `isOSMEnabledForState()`.    |
...
# 4. Generate OSM map data
bun scripts/generate-boundaries.ts --state=new-state
bun scripts/generate-coordinates.ts --state=new-state

# 5. Generate images (maintainers only)
